 ...
 * @c lambda_max(C_parent) be the maximum lambda reached anywhere in @c C_parent's subtree (either
 ...
 * @endverbatim
 *
 * where @c lambda(x) is the lambda at which @c x fell out of its containing cluster. The ratio is
 * bounded in @c [0, 1] by construction. A point that persists all the way to @c lambda_max scores
 ...
 * sub-cluster's own death lambda. This is the distinction that keeps scores in @c [0, 1].
 *
 ...
 * @param labels Per-point labels (result of EOM or leaf extraction). Noise points (@c -1) receive
