...
/**
 * @brief Contract for an MST backend satisfying the frozen @ref clustering::hdbscan::MstOutput
 * shape.
 *
 ...
 * into a caller-provided @ref clustering::hdbscan::MstOutput. Backends own their private scratch
 * and may amortize shape-indexed buffers across calls; per the HDBSCAN class invariant,
 * data-dependent indices (KDTree, kNN graph) are rebuilt per fit.
 *
 * The concept lives in its own header so that concrete backend implementations can depend on it
 * without pulling in the @ref clustering::HDBSCAN class template, and so the dispatcher can take
 * the concept as an input without introducing a header cycle.
 *
