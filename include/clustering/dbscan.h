...
public:
  ...
  void run(const NDArray<T, 2> &X) {
    ...
    // massively under-estimate DBSCAN work -- the backend does per-query tree walks that are
    // much heavier than `d` ops -- and caused @c n_jobs=16 at @c n=100k, @c d=2 to fall back
    // to serial here while every worker-side kernel would have happily cleared its own gate.
