...
def test_kmeans_n_init_1_matches_default() -> None:
    # n_init=1 must be bit-equal to the single-shot path: one restart with seed=s reproduces
    # kmeans(seed=s) exactly. Bytes-level equality on labels and centroids; hex-level equality
    # on the float64 inertia.
    for n, d, k, s in [
        (300, 8, 5, 0),
        (400, 4, 6, 7),
        (500, 12, 8, 99),
        ...
        labels, centroids, inertia, n_iter, converged = m.kmeans(
            X, k=k, max_iter=100, tol=1e-4, seed=s, n_jobs=1, n_init=1
        )
        _assert(
            np.array_equal(labels, ref_labels),
            f"labels diverge at (n={n}, d={d}, k={k}, s={s}) for n_init=1 vs default",
        )
        _assert(
            centroids.tobytes() == ref_centroids.tobytes(),
            f"centroids differ bitwise at (n={n}, d={d}, k={k}, s={s}) for n_init=1 vs default",
        )
        _assert(
...
def test_kmeans_n_init_returns_best_inertia_run() -> None:
    # Run the Python equivalent of n_init=N: loop seed..seed+N-1, collect each restart's
    # (inertia, labels, centroids, n_iter, converged), then assert that the single C++ call
    # with n_init=N returns the lowest-inertia entry's outputs (not an arbitrary restart's).
    ...
    bo_labels, bo_centroids, bo_inertia, bo_n_iter, bo_converged = m.kmeans(
        X, k=6, max_iter=100, tol=1e-4, seed=seed_first, n_jobs=1, n_init=n_init
    )
    _assert(
...
def test_kmeans_n_init_inertia_within_tolerance() -> None:
    # Best inertia from n_init=N differs from the equivalent Python loop's best by at most
    # 1e-6 * best_python_loop. With matched seeds and a reused solver instance the two must
    # agree bit-for-bit, but the explicit tolerance is what we lock in here.
    ...
    _, _, best_binding, _, _ = m.kmeans(
        X, k=8, max_iter=100, tol=1e-4, seed=seed_first, n_jobs=1, n_init=n_init
    )
...
def test_kmeans_n_init_validation() -> None:
    ...
    raised = False
    try:
        m.kmeans(X, k=3, n_init=0)
    except ValueError:
        raised = True
    _assert(raised, "kmeans with n_init=0 should raise ValueError")
...
def main() -> int:
    tests = [
        ...
        test_kmeans_n_init_1_matches_default,
        test_kmeans_n_init_returns_best_inertia_run,
        test_kmeans_n_init_inertia_within_tolerance,
        test_kmeans_n_init_validation,
    ]
    failures = 0
    for test in tests:
        try:
